...
u64 hashXXL(void* key, KeyType t){
    u64 v = 0;
    switch (t)
    {
        ...
        case MAP_KEY_TYPE_U8:
            v = hash_unsigned((u64)key,_XXL_MAP_SIZE);
            break;
        case MAP_KEY_TYPE_S64:
        case MAP_KEY_TYPE_S32:
        case MAP_KEY_TYPE_S16:
        case MAP_KEY_TYPE_S8:
            v = hash_signed((s64)key,_XXL_MAP_SIZE);
            break;
        case MAP_KEY_TYPE_F64:
            v = hash_f64(*((f64*)key),_XXL_MAP_SIZE);
            break;
        case MAP_KEY_TYPE_F32:
            v = hash_f32(*((f32*)key),_XXL_MAP_SIZE);
            break;
        case MAP_KEY_TYPE_STRING:
            v = hash_string((char*)key,_XXL_MAP_SIZE);
            break;
        default:
            break;
    }
...
bool unordered_map_xxl_create(XXLargeUnorderedMap* out_map, KeyType type){
    XXLargeUnorderedMap map = {};
    map.length = 0;
    for(u32 i=0; i< _XXL_MAP_SIZE; i++){
        MapNode mnode = {
            .key = 0,
            .next = 0,
            .prev = 0,
...
bool unordered_map_xxl_set(XXLargeUnorderedMap* map, void* key, void* value){
    u64 index = hashXXL(key,map->type);
...
bool unordered_map_xxl_clean(XXLargeUnorderedMap* map){
    for(u32 i=0; i< _XXL_MAP_SIZE; i++){
        MapNode mnode = {
            .key = 0,
            .next = 0,
