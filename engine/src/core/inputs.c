...
void input_process_key(keys key, bool pressed) {
    ...
    if (state.keyboard_current.keys[key] != pressed) {
        ...
        raise_event(pressed ? EVENT_CODE_KEY_DOWN : EVENT_CODE_KEY_UP, 0, context);
    }
}
...
void input_process_button(buttons button, bool pressed) {
    ...
    if (state.mouse_current.buttons[button] != pressed) {
        ...
        raise_event(pressed ? EVENT_CODE_BUTTON_DOWN : EVENT_CODE_BUTTON_UP, 0, context);
    }
}
...
void input_process_mouse_move(s16 x, s16 y) {
    ...
    if (state.mouse_current.x != x || state.mouse_current.y != y) {
        ...
        raise_event(EVENT_CODE_MOUSE_MOVE, 0, context);
    }
}
...
void input_process_mouse_wheel(s8 z_delta) {
    ...
    raise_event(EVENT_CODE_MOUSE_WHEEL, 0, context);
}
